...
download_backup() {
  ...
  local gcs_url="gs://$BUCKET_NAME/minecraft/$save_name.backup"
  ...
  if gsutil -q stat "$gcs_url"; then
    ...
    tar -xzvf "/tmp/backup.tar.gz" -C "/etc/minecraft"
    echo "Backup downloaded successfully."
  else
    echo "Backup for $save_name does not exist in GCS. Generating first-time backup."
    /etc/minecraft/generate_backup.sh "$save_name"
  fi
}
...
main() {
  ...
  /usr/bin/env java -jar /etc/minecraft/server.jar
